...
const GymDetail = () => {
  ...
  const temporarySuspendGym = async () => {
    try {
      const docRef = doc(db, "Gyms", gym.id);
      await updateDoc(docRef, { isApproved: "Pending" });
      toast.warn("Gym Suspended successfully");
      navigate("/gyms");
    } catch (error) {
  ...
  return (
    <div className="w-full p-4 gap-4 flex flex-row justify-center">
      <div className="w-8/12 bg-gray-50 shadow-md rounded-md p-4 justify-items-center [&>*]:container [&>*]:m-1">
        <h1 className="text-3xl font-bold">{gym.gym_name}</h1>
        <img
          src={gym.images[0] || "/assets/DefaultGym.png"}
          alt={gym.gym_name}
          className="hover:scale-105 duration-300 transition-all w-10/12 h-80 object-cover rounded-md my-10"
        />
        <p className=" text-lg">Description: {gym.description}</p>
        ...
        <p className="my-4 gap-2 flex justify-start items-center">
          <span>Gym Type:</span>
          <span
            className={`!text-start text-gray-700 ${
              gym.gymType ? "px-3 py-2 rounded-md bg-green-500 text-white" : ""
            }`}
          >
        ...
        <button
          onClick={() => temporarySuspendGym()}
          className="px-4 py-2 bg-red-500 text-white font-semibold rounded-lg shadow-md hover:bg-red-600 active:outline-none active:ring-2 active:ring-red-400 active:ring-offset-2"
        >
          Suspend GYM
        </button>
        <div className="border-t w-full p-4">
          ...
          <div className="flex flex-col justify-center gap-2">
            {gym.opening_hours.length > 0 ? (
              gym.opening_hours.map((data, index) => (
                <h1
                  key={index}
                  className="text-gray-700 text-lg mt-4 text-center"
                >
                  {data}
                </h1>
              ))
            ) : (
              <h1 className="text-gray-700 text-lg mt-4 text-center">
                9:00 AM - 06:00 PM
              </h1>
            )}
          </div>
      ...
      <div className="w-4/12 bg-gray-50 shadow-md justify-items-center rounded-md">
        <button
          onClick={() => navigate(`/transactions/${gym.id}`)}
          className="hover:shadow-lg hover:bg-gray-300 flex flex-row bg-gray-200 items-center rounded-full m-4  px-4 py-2 w-4/5 justify-center gap-2 font-semibold"
        >
          Trnasactions History {<FaArrowAltCircleRight />}
        </button>
        ...
        <div className="border-t w-full p-4">
          <h1 className="text-gray-700 text-lg text-center">
            Total Visitors: {gym.visitors?.length}
          </h1>
        </div>
        <div className="border-t w-full p-4 flex flex-row justify-center gap-2">
          <p className="text-center text-gray-700">Rated:</p>
          <p className="text-center text-yellow-500 flex flex-row items-center">
            {gym.ratings}
            {<FaStar />}
          </p>
        </div>
